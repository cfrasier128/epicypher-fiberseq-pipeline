...
process get_fire_pileups_no_shuffle{
    ...
    tuple val(sampname), path(fire_bam), val(ref_name), path(fire_bai)
    output:
    tuple val(sampname), path("*.pileup.bed.gz"), val(ref_name)
    script:
    """
...
process make_fdr_bed {
    ...
    input:
    tuple val(sampname), path(fdr_table), val(ref_name), path(no_shuffle_pileup)
    output:
    tuple val(sampname), path("*.FDR.bed.gz"), val(ref_name), path("*.FDR.bed.gz.tbi")

    script:
    """
    conda run -n FIRE python3 /opt/FIRE/scripts/fdr-table.v2.py -v 1 \
    --fdr-table $fdr_table \
    $no_shuffle_pileup ${sampname}.FIRE_dev.FDR.bed;
    bgzip -@ 8 ${sampname}.FIRE_dev.FDR.bed;
    tabix -p bed ${sampname}.FIRE_dev.FDR.bed.gz
    """
}
...
process get_only_FIREs {
   ...
   container 'cfrasier/epi-fire'
    ...
    input:
    tuple val(sampname), path(fdr_bed), val(ref_name), path(fdr_bed_tbi)
    output:
    tuple val(sampname), path("*.FIREs.bed.gz"), val(ref_name)
    script:
    """
...
workflow call_fire_peaks {
    ...
    // make_fdr_bed_input_ch.view()
    // 0sampname, 1fdr_table, 2ref_name, 3shuffle_pileup_bed

    make_fdr_bed(make_fdr_bed_input_ch)
